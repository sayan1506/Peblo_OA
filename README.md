...
Viewer (Part C) is in progress: Phase C1 (project skeleton + data layer) is complete - a separate `viewer/` app with zero auth/token concept, reading only the two public `/catalog` and `/catalog/search` routes. `fetchCatalog`/`searchCatalog` wrap a shared `CatalogError`-throwing fetch helper; TanStack Query hooks (`useCatalog`, `useSearch`) sit on top. Phase C2 (home hero + rows) is also complete - the home page now renders a real hero (banner artwork, title, synopsis, link into the show) from the first catalog section's first show, plus one horizontally-scrollable row per section with poster-artwork tiles; a shared `ArtworkImage` component renders an accessible "No artwork" placeholder wherever a show/episode has no uploaded artwork for that kind, reserving layout space via `aspect-ratio` to avoid shift on load. Phase C3 (show detail) is also complete - the show page renders poster artwork, synopsis, categories, each season's episodes in order (thumbnail, title, `mm:ss` duration, language badges) and a separate "Trailers" section for season 0, all found client-side in the already-fetched catalogue with no extra network call. Phase C4 (search and filters) is also complete - a search page with a debounced text box plus section/category/language dropdowns, all backed by `GET /catalog/search` and mirrored in the URL; results render grouped by show (in the backend's own row order) rather than pretending to reconstruct a season/episode structure the flat search response doesn't carry. CI is still to come.
