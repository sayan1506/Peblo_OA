...
CMS (Part B) is underway: the project skeleton with login/JWT/authenticated API client, a shows list (search, section/status/category filters, pagination, URL-persisted state), a show detail page with a seasons -> episodes drill-down, create/edit forms for shows, seasons (add-only), and episodes with real backend validation messages surfaced inline, a three-slot artwork uploader (poster/banner/thumbnail) on the show/episode edit forms with live preview and real validation errors, and a publish page (validation report, a publish button gated by role and blocking issues, and paginated run history backed by a new `GET /admin/publish-runs` endpoint). The remaining UI-states polish pass, the viewer, and CI are still to come.
