...
Viewer (Part C) is in progress: Phase C1 (project skeleton + data layer) is complete - a separate `viewer/` app with zero auth/token concept, reading only the two public `/catalog` and `/catalog/search` routes. `fetchCatalog`/`searchCatalog` wrap a shared `CatalogError`-throwing fetch helper; TanStack Query hooks (`useCatalog`, `useSearch`) sit on top. Phase C2 (home hero + rows) is also complete - the home page now renders a real hero (banner artwork, title, synopsis, link into the show) from the first catalog section's first show, plus one horizontally-scrollable row per section with poster-artwork tiles; a shared `ArtworkImage` component renders an accessible "No artwork" placeholder wherever a show/episode has no uploaded artwork for that kind, reserving layout space via `aspect-ratio` to avoid shift on load. Search and show detail still render raw JSON for now (C3/C4). CI is still to come.
