...
CMS (Part B) is underway: the project skeleton with login/JWT/authenticated API client, a shows list (search, section/status/category filters, pagination, URL-persisted state), a show detail page with a seasons -> episodes drill-down, and create/edit forms for shows, seasons (add-only), and episodes with real backend validation messages surfaced inline. Artwork upload and the publish page are still to come. The viewer and CI are still to come.
