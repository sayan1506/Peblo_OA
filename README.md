...
CMS (Part B) is underway: the project skeleton with login/JWT/authenticated API client, plus a shows list (search, section/status/category filters, pagination, URL-persisted state) and a show detail page with a seasons -> episodes drill-down. Create/edit forms, artwork upload, and the publish page are still to come. The viewer and CI are still to come.
