...
Part E (written answers) is complete - see below.

## Time spent

- Part A (backend): ~8 hours
- Part B (CMS): ~6 hours
- Part C (viewer): ~5 hours
- Part D (pipeline and operability): ~2 hours
- Part E (written answers): ~1 hour
...
**Publish atomicity / mid-publish death.** A `PublishRun` row is created (`outcome: "running"`) before the build starts. The atomicity itself lives in `LocalStorage.put()`: new JSON is written to a temp file, then `os.replace()`d over `catalog.json` - one filesystem rename, so a concurrent `GET /catalog` always sees a fully-old or fully-new file, never a partial one. If the process dies mid-build, the temp file is orphaned and `catalog.json` is untouched - readers keep the last good publish. The real gap: a run that dies mid-build stays stuck at `"running"` forever, since nothing marks it failed after the fact - a production version needs a watchdog for stale "running" rows.

**Storage abstraction: local disk to R2.** `Storage` (`app/storage/base.py`) is a 3-method ABC (`put`/`get`/`delete`); `LocalStorage` is the only implementation, chosen via `settings.storage_backend`. Moving to R2 is one new `R2Storage(Storage)` class using `boto3` against R2's S3-compatible endpoint, plus a branch in `get_storage()`. Two real differences beyond the class swap: R2's `PutObject` is already atomic, so the temp-file-plus-rename dance becomes unnecessary; and every `/static/...` URL (`artwork_url_map()`, the catalogue's `artwork` fields) would need to become a public bucket URL or presigned URL, since the API no longer serves the bytes itself.

**Search: implementation, scaling limit, next step.** `GET /catalog/search` re-parses the full published `catalog.json` on every request and linearly scans it, applying `q`/`category`/`language`/`section` as plain filters (`q` a case-insensitive substring match, all AND together). Sub-millisecond at this seed size (95 episodes). It starts to hurt in the low thousands of episodes, since every request re-parses the whole file with no caching between requests; a real Netflix-scale catalogue makes a per-request linear scan a non-starter. First (cheap) fix: cache the parsed catalogue in memory, invalidated on publish. Real fix: a proper index (Postgres full-text/trigram, or Meilisearch) built once per publish instead of scanned per request.

**Catalogue file vs. querying the DB per request.** Reading only the published file decouples the public surface from database load/availability and guarantees every reader sees one atomically-published snapshot, never a half-edited one. Where it bites: staleness is a real user-facing fact, not an implementation detail - an editor who forgets to hit Publish has changed nothing the public sees. It's also not a strategy that scales forever - a much bigger catalogue needs the same fix search does (cache/index). And it's a second source of truth: if the file and DB ever disagree, the only fix is a full re-publish, no incremental repair.

**What was left out.** Token-refresh/expiry UX (a 401 today is a generic error - the brief only asked for wrong-*role* enforcement, not expiry). A full 401/403 test matrix and a transactional test-DB fixture - the riskiest logic already has targeted tests; broader coverage wasn't worth the time here. Per-slot artwork replacement - re-uploading a kind adds a new row rather than replacing the old one, so old rows accumulate silently. All three optional stretch goals were attempted (see below).

**Stretch goal: publish dry-run diff.** `GET /admin/catalog/dry-run` (editor-gated) calls the same `build_catalog()` the real publish uses, but never writes anything - it diffs the result against whatever `catalog.json` currently has published (or treats everything as new if nothing's published yet), returning shows/episodes added, removed, and changed (by field). The CMS publish page has a "Preview changes" button that renders this before anyone commits to a real publish. Diffing is by `slug`/`content_group`, matching how the catalogue itself is keyed - and it already caught a real case in this repo: after reseeding, the dry-run correctly reported that the previously-published "Moti's Many Lives" would be *removed*, since seeding resets shows back to `draft` and publishing is a separate explicit action.

**Stretch goal: versioned catalogue with rollback.** Each `PublishRun` now stores the full built catalogue it wrote, in a `catalog_snapshot` JSON column - not a separate versioned-files scheme in `Storage`, since the catalogues here are small and `publish_runs` was already the durable history of every publish. A new admin-only `POST /admin/catalog/rollback/{run_id}` loads a prior run's snapshot and writes it to `catalog.json` through the exact same atomic `storage.put()` path a real publish uses, then records the rollback as a *new* run (`outcome: "rolled_back"`, `rolled_back_from_id` pointing at the source run) rather than mutating history in place - consistent with publish-runs only ever appending. A run with no snapshot (a `failed` or `running` row, or anything published before this migration) 409s instead of silently no-opping. The CMS run-history table gets a "Roll back" column, admin-only, gated behind a native `confirm()` (this app has no modal component yet, so a browser confirm is the minimal fit for one destructive action) - clicking it rolled back a real publish live against the running stack and correctly restored the exact prior catalogue content.

**Stretch goal: audit log of who changed what.** A new `AuditLog` table (`actor_id`, `action`, `resource_type`, `resource_id`, `summary`, `created_at`) gets one row per mutating CRUD call across shows/seasons/episodes/artwork, written via a shared `record_audit_log()` helper called explicitly at each route handler's existing commit point (not a SQLAlchemy event hook) - explicit call sites let each entry's summary say something specific ("updated show 'X' (section)"), not a generic diff. `GET /admin/audit-log` is admin-only, unlike the validation report and publish-runs endpoints, since this is a supervisory view of what the whole team has been doing, not something an editor needs to do their own job. The CMS gets a new `/audit-log` page (admin-only nav link and route) listing actor, action, resource, and summary, paginated the same way run history already is. Verified live: creating, editing, and deleting a show produced exactly the three expected rows with the correct actor and readable summaries, and an editor got a 403 from the API and a permission-denied page in the CMS with no nav link.

**AI tools used.** Built with Claude Code end to end, run as reviewed pair-programming rather than autopilot: each phase was scoped and approved before implementation, and specific decisions (`categories` as an array column vs. a join table, adding `GET /admin/publish-runs`) were confirmed against the source docs before writing code. Concrete corrections along the way: a `docker-compose.yml` bug where `DATABASE_URL` pointed at the host-side Postgres mapping instead of the compose-network one, caught with `docker compose config` before ever starting a container; a Playwright script that reported a false "viewer regression," traced to a bug in the script itself, not a real one; a handful of `ruff` findings (missing `TYPE_CHECKING` imports for forward-referenced model relationships) fixed directly. Nothing generated shipped unread, and design-level choices were confirmed, not assumed.
